...
NORFLU <- sf::st_read("map.shp")
UCs_NF <- sf::st_read("UCs_NF.shp")
...
dados <- read.csv("data/monitoramento_UENF.csv", header = TRUE) %>% 
  mutate(DATA = as.POSIXct(DATA, format = "%Y-%m-%d"))


riqueza <- dados %>% 
  group_by(local, DATA) %>% 
  summarise(spp = n_distinct(taxon)) 
...
dados <- read.csv("data/monitoramento_UENF.csv", header = TRUE) %>% 
  mutate(DATA = as.POSIXct(DATA, format = "%d/%m/%Y"))
